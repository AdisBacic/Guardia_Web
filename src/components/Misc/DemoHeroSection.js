import React from 'react';

const DemoHeroSection = () => {
  return (
    <div className="text-center py-20 bg-gray-00">
      <h1 className="mb-6 text-4xl font-bold text-indigo-700 dark:text-indigo-200 lg:text-5xl">
        Upptäck <span className="text-indigo-500">Enkelheten</span> och <span className="text-indigo-500">Effektiviteten</span>
      </h1>
      <p className="mx-auto max-w-2xl text-lg font-medium text-gray-600 dark:text-gray-300">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris      </p>
    </div>
  );
};

export default DemoHeroSection;
